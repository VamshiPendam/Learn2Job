...
const ToolDiscovery: React.FC = () => {
   ...
   return (
      <div className="flex flex-col md:flex-row md:h-[calc(100vh-80px)]">
         ...
         {/* ── Sidebar ─────────────────────────────────────────── */}
         <aside className="w-full md:w-64 flex-shrink-0 bg-[#0a0f12] border-b md:border-b-0 md:border-r border-[#1e293b]">

            {/* Mobile: horizontal scrollable category chips */}
            <div className="flex md:hidden overflow-x-auto gap-2 p-3">
               {categories.map(c => (
                  <button
                     key={c}
                     onClick={() => setSelectedCategory(c)}
                     className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${selectedCategory === c ? 'bg-[#00f2ea] text-[#060a0d]' : 'bg-[#131b21] text-gray-400 border border-[#1e293b]'}`}
                  >
                     {c}
                  </button>
               ))}
            ...
            <div className="hidden md:flex flex-col p-6 space-y-8 overflow-y-auto h-full">
               {/* Categories */}
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                     {pricingOptions.map(p => (
                        <label key={p.value} className="flex items-center space-x-3 group cursor-pointer" onClick={() => togglePricing(p.value)}>
                           <div className={`w-5 h-5 rounded border transition-colors flex items-center justify-center ${selectedPricing.includes(p.value) ? 'border-[#00f2ea] bg-[#00f2ea]/10' : 'border-[#1e293b] group-hover:border-[#00f2ea]'}`}>
                              <i className={`fas fa-check text-[10px] text-[#00f2ea] transition-opacity ${selectedPricing.includes(p.value) ? 'opacity-100' : 'opacity-0'}`}></i>
                           </div>
                           <span className={`text-sm transition-colors ${selectedPricing.includes(p.value) ? 'text-white font-bold' : 'text-gray-400 group-hover:text-white'}`}>{p.label}</span>
                        </label>
                     ))}
                  </div>
               </div>
               ...
               <div className="space-y-4 pt-8">
                  ...
                  <button
                     onClick={handleSync}
                     disabled={isSyncing}
                     className={`w-full py-4 rounded-2xl font-bold text-sm transition-all flex items-center justify-center space-x-3 ${isSyncing ? 'bg-[#131b21] text-gray-500 cursor-not-allowed border border-[#1e293b]' : 'bg-[#00f2ea] text-[#060a0d] hover:scale-[1.02] shadow-[0_0_20px_rgba(0,242,234,0.2)]'}`}
                  >
         ...
         {/* ── Main Grid ────────────────────────────────────────── */}
         <main className="flex-1 bg-[#060a0d] p-4 md:p-8 overflow-y-auto">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6 md:mb-8">
               ...
               <div className="flex items-center space-x-3">
                  {/* View mode toggle */}
                  <div className="flex items-center space-x-1 bg-[#131b21] p-1.5 rounded-xl border border-[#1e293b]">
                     <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'text-[#00f2ea] bg-[#0a0f12] border border-white/5 shadow-lg' : 'text-gray-500 hover:text-white'}`}>
                        <i className="fas fa-th-large"></i>
                     </button>
                     <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'text-[#00f2ea] bg-[#0a0f12] border border-white/5 shadow-lg' : 'text-gray-500 hover:text-white'}`}>
                  ...
                  <div className="relative">
                     <div onClick={() => setIsSortOpen(!isSortOpen)} className="bg-[#131b21] px-4 py-2 rounded-xl border border-[#1e293b] text-sm text-gray-400 flex items-center space-x-2 cursor-pointer hover:border-[#00f2ea]/50 transition-all select-none">
                     ...
                     {isSortOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-[#131b21] border border-[#1e293b] rounded-2xl shadow-2xl py-2 z-[100] backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
                           {(['Trending', 'Top Rated', 'Newest'] as const).map((option) => (
                              <button key={option} onClick={() => { setSortBy(option); setIsSortOpen(false); }} className={`w-full text-left px-4 py-2.5 text-sm font-semibold transition-all hover:bg-white/5 ${sortBy === option ? 'text-[#00f2ea]' : 'text-gray-400'}`}>
                                 {option}
                              </button>
                           ))}
                        </div>
            ...
            {viewMode === 'grid' ? (
                  ...
                  {paginatedTools.map(tool => (
                     <div key={tool.id} className="bg-[#131b21] border border-[#1e293b] rounded-3xl p-6 group hover:border-[#00f2ea]/40 transition-all flex flex-col h-full relative overflow-hidden">
                        ...
                        <div className="flex flex-wrap gap-2 mb-8 relative z-10">
                           <span className="px-2 py-0.5 bg-[#00f2ea]/10 border border-[#00f2ea]/20 rounded-md text-[9px] font-black text-[#00f2ea] uppercase tracking-tighter">{tool.category}</span>
                           {tool.tags.map(t => (
                              <span key={t} className="px-2 py-0.5 bg-white/5 border border-white/10 rounded-md text-[9px] font-bold text-gray-500 uppercase tracking-tighter">{t}</span>
                           ))}
                        </div>
                        <div className="flex items-center justify-between pt-6 border-t border-white/5 relative z-10">
                           <div className="flex flex-col">
                              <span className="text-[8px] text-gray-500 font-bold uppercase tracking-widest">Pricing</span>
                              <span className={`text-[10px] font-black uppercase tracking-widest ${tool.pricing === 'Free' ? 'text-green-500' : tool.pricing === 'Freemium' ? 'text-blue-400' : 'text-purple-400'}`}>{tool.pricing}</span>
                           </div>
                           <a href={tool.url} target="_blank" rel="noopener noreferrer" className="text-white text-xs font-bold hover:text-[#00f2ea] flex items-center space-x-2 p-2 hover:bg-white/5 rounded-lg transition-all">
                              <span>View Tool</span>
               ...
               <div className="flex flex-col space-y-4">
                  {paginatedTools.map(tool => (
                     <div key={tool.id} className="bg-[#131b21] border border-[#1e293b] rounded-2xl p-4 group hover:border-[#00f2ea]/40 transition-all flex items-center space-x-4 md:space-x-6 relative overflow-hidden">
                        ...
                        <div className="w-14 h-14 rounded-xl bg-[#0a0f12] border border-[#1e293b] flex-shrink-0 flex items-center justify-center text-[#00f2ea] text-2xl group-hover:shadow-[0_0_15px_rgba(0,242,234,0.3)] transition-all">
                           <i className={`fas ${tool.icon || 'fa-brain'}`}></i>
                        </div>
                        ...
                        <div className="hidden lg:flex flex-col items-start w-32 flex-shrink-0">
                           ...
                           <span className="px-2 py-0.5 bg-[#00f2ea]/10 border border-[#00f2ea]/20 rounded-md text-[9px] font-black text-[#00f2ea] uppercase truncate max-w-full">{tool.category}</span>
                        </div>
                        ...
                        <a href={tool.url} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 bg-[#1e293b] text-white px-3 md:px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#00f2ea] hover:text-[#060a0d] transition-all flex items-center space-x-2">
                           <span>View</span>
            ...
            {sortedAndFilteredTools.length === 0 && (
               <div className="flex flex-col items-center justify-center py-24 text-center">
                  ...
                  <p className="text-gray-500 max-w-xs">
                     {searchQuery ? `No tools match "${searchQuery}". Try different keywords or check your filters.` : "Adjust your filters or try syncing latest data from the market."}
                  </p>
                  {searchQuery && <p className="mt-6 text-[#00f2ea] text-sm font-bold">Use the top search bar to refine keywords</p>}
               </div>
            )}
            ...
            {sortedAndFilteredTools.length > pageSize && (
               <div className="flex items-center justify-center space-x-2 mt-12 pb-8">
                  <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className={`w-10 h-10 rounded-xl bg-[#131b21] flex items-center justify-center transition-colors ${currentPage === 1 ? 'text-gray-700 cursor-not-allowed' : 'text-gray-500 hover:text-white'}`}>
                     <i className="fas fa-chevron-left text-xs"></i>
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                     <button key={page} onClick={() => setCurrentPage(page)} className={`w-10 h-10 rounded-xl font-bold text-sm transition-all ${currentPage === page ? 'bg-[#00f2ea] text-[#060a0d]' : 'bg-[#131b21] text-gray-400 hover:bg-[#1e293b]'}`}>
                        {page}
                     </button>
                  ))}
                  <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className={`w-10 h-10 rounded-xl bg-[#131b21] flex items-center justify-center transition-colors ${currentPage === totalPages ? 'text-gray-700 cursor-not-allowed' : 'text-gray-500 hover:text-white'}`}>
